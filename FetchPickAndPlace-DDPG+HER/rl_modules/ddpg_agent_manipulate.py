...
class ddpg_agent:
    ...
    def learn(self):
        ...
        for epoch in range(self.args.n_epochs):  # 50
            for episode in range(self.args.n_cycles):  # 50
                ...
                for _ in range(self.args.num_rollouts_per_mpi):  # 2
                    ...
                    while len(ep_obs) < 50:
                        ...
                        """
                        Hand-enginereed code for approach
                        """
                        timeStep = 0
                        ...
                        '''
                        train manipulate part
                        store actions
                        '''
                        
                        while np.linalg.norm(grip_pos - objectPos) >= 0.015 and timeStep < self.env._max_episode_steps:
                            with torch.no_grad():
                                input_tensor = self._preproc_inputs(obs, objectPos)
                                pi = self.actor_network(input_tensor)
                                action = self._select_actions(pi)
                            
                            #self.env.render()
                            action[3]= -0.01
                            observation_new, reward, terminated, truncated, info = self.env.step(action, objectPos)

                           
                            'calculate th new gripper position after one step action and update achieved_goal'
                ...
                # store the episodes
                self.bufferManipulate.store_episode([mb_obs, mb_ag, mb_g, mb_actions])
                self._update_normalizer([mb_obs, mb_ag, mb_g, mb_actions])

                
                for _ in range(self.args.n_batches):
                    # train the network
                    self._update_network("manipulate")
                # soft update
            ...
            if success_rate > 98:
                torch.save([self.o_norm.mean,self.o_norm.std,self.g_norm.mean,self.g_norm.std,self.actor_network.state_dict()],self.model_path + "/manipulate.pt")
    ...
    def _eval_agent(self):
        ...
        for test in range(100):  # 10
            ...
            '''
            while np.linalg.norm(object_oriented_goal) >= 0.003 and timeStep <= self.env._max_episode_steps:
                #self.env.render()
                action = [0, 0, 0, 0]
                object_oriented_goal = object_rel_pos.copy()
                object_oriented_goal[2] += 0.03
            ...
            while np.linalg.norm(grip_pos - objectPos) >= 0.015 and timeStep <= self.env._max_episode_steps:
                ...
                observation_new, _, _, _, info = self.env.step(actions, objectPos)
            ...
            '''
            Hand-engineered solution for retract
            '''
